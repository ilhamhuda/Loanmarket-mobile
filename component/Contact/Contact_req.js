...
class BuyerWall extends Component {
  static navigationOptions = {
   ...
   componentWillMount(){		
    api.getAkses(this.state.contactid).then((result)=>{ 
      ...
      submit(id){
		api.aksesdelete(id).then((result)=>{ 
	      ToastAndroid.show(result.message,ToastAndroid.SHORT);
	      if(result.status) Alert.alert(id + ' coy');
	    });
	}
	constructor(props) {    
    	super(props);     
    ...
    const cekid = params ? params.id : null;	 
    	this.state = { active: false,contactid:cekid,request:[],listingproduk:[],mybuyer:[
     ...
     <List style={{backgroundColor: '#00a8ea', height:45, borderRadius:10, marginBottom:10}}>
            ...
            <ListItem style={{ paddingTop:-10}}>
              <Left>
               ...
               <Button transparent onPress={()=>this.props.navigation.navigate('Contactuser',{id:this.state.contactid})}>
              
              <Text style={{color:"white"}}>Add Access</Text>
              </Button>
              </Left>
              <Right>
              <Button transparent onPress={()=>this.props.navigation.navigate('Contactuser',{id:this.state.contactid})}>
